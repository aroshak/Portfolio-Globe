...
export function GitRepoScroller() {
  ...
  const move = (direction: number) => {
    targetRef.current = Math.max(0, Math.min(githubRepos.length - 1, Math.round(targetRef.current + direction)));
  ...
  useEffect(() => {
    ...
    const meshes = githubRepos.map((repo, index) => {
    ...
    const onMove = (e: PointerEvent) => { if (!renderer.domElement.hasPointerCapture(e.pointerId)) return; const d=(downX-e.clientX)/260; if(Math.abs(d)>.05) dragged=true; targetRef.current=Math.max(0,Math.min(githubRepos.length-1,startTarget+d)); };
    ...
    const onWheel = (e: WheelEvent) => { e.preventDefault(); targetRef.current=Math.max(0,Math.min(githubRepos.length-1,targetRef.current+(e.deltaY+e.deltaX)*.0025)); setActive(Math.round(targetRef.current)); };
  ...
  return <section className="repo-showcase" aria-labelledby="repo-showcase-title">
    ...
    <div className="repo-tabs page-shell">{githubRepos.map((repo,i)=><button className={i===active?"active":""} key={repo.id} onClick={()=>{targetRef.current=i;setActive(i)}}><GitBranch size={12}/>{repo.name}</button>)}</div>
